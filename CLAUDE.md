...
- The previous **imported** cert (arn `eabee32f`, valid to 2039) is no longer managed by Terraform (`state rm`'d, left alive in ACM). Both it and the new cert show `InUseBy` load balancers named `prod-nrt-1-cdtls-*` in account `969236854626`. That is **AWS's own API Gateway service account for `ap-northeast-1`** (confirmed against public known-AWS-account lists) — the `cdtls` ("Custom Domain TLS") ALBs are API Gateway's managed fleet that terminates TLS for regional custom domains. So any ACM cert attached to an API Gateway custom domain shows in-use by that AWS-owned fleet: expected, not a third party, no external access to our account. Consequence: the cert auto-renews in place fine, but **cannot be deleted/replaced while the custom domain still references it** (`terraform destroy` fails with `ResourceInUseException` — this is what stalled the first migration apply).
